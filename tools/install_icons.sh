...
SIZE_STANDARD_PX="${SIZE_STANDARD_PX:-24}"
SIZE_PRIMARY_PX="${SIZE_PRIMARY_PX:-32}"
SIZE_LARGE_PX="${SIZE_LARGE_PX:-48}"

# Source and output defaults
TABLER_BASE_URL="${TABLER_BASE_URL:-https://raw.githubusercontent.com/tabler/tabler-icons/master/icons/outline}"
OUT_DIR="${OUT_DIR:-src/wasm/generated_icons}"
OUT_BASENAME="${OUT_BASENAME:-tabler_icons}"

usage() {
    cat <<'EOF'
Usage:
  tools/install_icons.sh <icons_list_file>

Description:
  Reads icon names (one per line) from <icons_list_file>, downloads matching
  Tabler SVG icons, normalizes stroke width, rasterizes them, and generates
  LVGL-ready C assets with three variants per icon:
    _S = standard (24px)
    _P = primary  (32px)
    _L = large    (48px)
...
Example:
  tools/install_icons.sh icons_list.txt
  STROKE_WIDTH=1.75 SIZE_PRIMARY_PX=36 tools/install_icons.sh icons_list.txt
EOF
}
...
if [[ ! -f "$ICON_LIST_FILE" ]]; then
    echo "Error: icon list file not found: $ICON_LIST_FILE" >&2
    exit 1
fi
...
python3 - "$ICON_LIST_FILE" "$TABLER_BASE_URL" "$OUT_DIR" "$OUT_BASENAME" "$STROKE_WIDTH" "$SIZE_STANDARD_PX" "$SIZE_PRIMARY_PX" "$SIZE_LARGE_PX" <<'PY'
import io
import os
import re
import sys
...
try:
    ...
    print("Error: python module 'cairosvg' is required. Install with: pip install cairosvg", file=sys.stderr)
    raise

try:
    from PIL import Image
except Exception:
    print("Error: python module 'Pillow' is required. Install with: pip install pillow", file=sys.stderr)
    raise
...
VARIANTS = [
    ("S", int(size_standard_px)),
    ("P", int(size_primary_px)),
    ("L", int(size_large_px)),
]
...
for icon_name in icons:
    url = f"{base_url.rstrip('/')}/{icon_name}.svg"
    try:
        with urllib.request.urlopen(url) as resp:
            svg_raw = resp.read().decode("utf-8")
    except urllib.error.HTTPError as e:
        print(f"Error: failed to download '{icon_name}' ({url}): HTTP {e.code}", file=sys.stderr)
        sys.exit(2)
    except Exception as e:
        print(f"Error: failed to download '{icon_name}' ({url}): {e}", file=sys.stderr)
        sys.exit(2)
...
header_lines = []
header_lines.append("/* Auto-generated by tools/install_icons.sh. Do not edit manually. */")
...
header_lines.append("")
header_lines.append("const tabler_icon_dsc_t *tabler_icon_by_name_variant(const char *name, char variant);")
header_lines.append("const tabler_icon_dsc_t *tabler_icon_by_name_size(const char *name, uint16_t size_px);")
header_lines.append("")
header_lines.append("#ifdef __cplusplus")
header_lines.append("}")
header_lines.append("#endif")
...
source_lines = []
source_lines.append("/* Auto-generated by tools/install_icons.sh. Do not edit manually. */")
...
source_lines.append("")
source_lines.append(
    "const uint32_t tabler_icon_registry_count = (uint32_t)(sizeof(tabler_icon_registry) / sizeof(tabler_icon_registry[0]));"
)
source_lines.append("")
source_lines.append("const tabler_icon_dsc_t *tabler_icon_by_name_variant(const char *name, char variant) {")
source_lines.append("    if(name == NULL) return NULL;")
source_lines.append("    for(uint32_t i = 0; i < tabler_icon_registry_count; i++) {")
source_lines.append("        if(icon_name_equals(name, tabler_icon_registry[i].name) && variant == tabler_icon_registry[i].variant) {")
...
source_lines.append("}")
source_lines.append("")
source_lines.append("const tabler_icon_dsc_t *tabler_icon_by_name_size(const char *name, uint16_t size_px) {")
source_lines.append("    if(name == NULL) return NULL;")
source_lines.append("    for(uint32_t i = 0; i < tabler_icon_registry_count; i++) {")
source_lines.append("        if(icon_name_equals(name, tabler_icon_registry[i].name) && size_px == tabler_icon_registry[i].size_px) {")
source_lines.append("            return tabler_icon_registry[i].icon;")
source_lines.append("        }")
source_lines.append("    }")
source_lines.append("    return NULL;")
...
print(
    ...
    f"(S={VARIANTS[0][1]}px, P={VARIANTS[1][1]}px, L={VARIANTS[2][1]}px) | Stroke: {stroke_width}"
)
PY
