...
var page_containers: [PAGE_COUNT]?*lv.lv_obj_t = .{ null, null, null, null };

// Nav icon button objects (for highlight tracking)
var nav_buttons: [PAGE_COUNT]?*lv.lv_obj_t = .{ null, null, null, null };
...
fn createNavBar(parent: ?*lv.lv_obj_t) void {
    ...
    const page_indices = [PAGE_COUNT]usize{ PAGE_LOGBOOK, PAGE_ANCHOR, PAGE_SAILS, PAGE_SETTINGS };
...
fn createPages(parent: ?*lv.lv_obj_t) void {
    ...
    for (0..PAGE_COUNT) |i| {
        ...
        switch (i) {
            ...
            PAGE_SETTINGS => createSettingsPage(container),
            else => {},
        }
    }
}
...
fn createSettingsPage(parent: ?*lv.lv_obj_t) void {
    if (parent == null) return;

    createPageTitle(parent, "Settings", PAGE_SETTINGS);
    ...
    if (bar) |b| {
        ...
        lv.lv_obj_set_style_bg_color(b, lv.lv_color_hex(COL_ACCENT_2), lv.LV_PART_ITEMS);
        lv.lv_obj_set_style_bg_opa(b, lv.LV_OPA_50, lv.LV_PART_ITEMS);
        lv.lv_obj_set_style_radius(b, 8, lv.LV_PART_ITEMS);
...
/// Completed callback — fires when the 4s animation reaches 100%.
fn powerOffAnimCompletedCb(anim: ?*lv.lv_anim_t) callconv(.C) void {
    ...
    if (platform_callbacks.power_off) |cb| {
        cb();
    }
...
/// PRESSED event — start the 4-second animation on the bar.
fn powerOffPressedCb(e: ?*lv.lv_event_t) callconv(.C) void {
    ...
    // lv_anim_t is opaque to Zig (contains bitfields/union), so we use a
    // static byte buffer as backing storage and cast the pointer.
    ...
    lv.lv_anim_set_duration(anim_ptr, 4000);
